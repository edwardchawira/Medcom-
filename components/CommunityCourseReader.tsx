...
export function CommunityCourseReader({
  ...
  useEffect(() => {
    ...
    async function loadCertificate() {
      try {
        const response = await fetch(`/api/certificates/community/${slug}`, { cache: "no-store" });
  ...
  const next = useCallback(() => {
    if (isAssessment && assessmentQuestions.length > 0 && !certificate) {
      alert("Submit and pass the final assessment to unlock your certificate.");
      return;
    }
    advanceToNextStep();
  }, [advanceToNextStep, assessmentQuestions.length, certificate, isAssessment]);
    ...
    <div ref={playerRef}>
      ...
      >
          ...
          <ProgressReporter
            ...
            courseTitle={courseTitle}
            status={isAssessment && isLast && (assessmentQuestions.length === 0 || Boolean(certificate)) ? "completed" : "in_progress"}
            progress={pct}
            resumePath={
              isAssessment
                ? `/courses/community/${slug}/learn/assessment`
            ...
            <>
              ...
              >
                ...
                {quizQuestions.length > 0 ? (
                  <ChapterQuiz title={`${ch?.title ?? "Chapter"} knowledge check`} questions={quizQuestions} />
                ) : null}
              </ActiveLearningChrome>
            </>
          ) : null}
...
function CommunityFinalAssessment({
  slug,
  courseTitle,
  questions,
  certificate,
  ...
  async function submit() {
    ...
    setSubmitting(true);
    try {
      const response = await fetch(`/api/certificates/community/${slug}`, {
        method: "POST",
      ...
      setSubmitError(err instanceof Error ? err.message : "Could not issue certificate.");
    } finally {
      setSubmitting(false);
    }
  }
  ...
  return (
    <section className="space-y-5">
      <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        <p className="text-xs font-bold uppercase tracking-wide text-teal-700">Final assessment</p>
        <h2 className="mt-2 text-2xl font-semibold tracking-tight text-slate-950">
          Pass the course to unlock your certificate
        </h2>
        <p className="mt-2 text-sm leading-relaxed text-slate-600">
          Answer all questions, submit once, review your result, retry if needed, and receive a certificate
          after meeting the pass mark.
        </p>
        <div className="mt-4 grid gap-3 text-sm sm:grid-cols-3">
          ...
          <div className="rounded-lg bg-slate-50 p-3 ring-1 ring-slate-200">
            ...
            <p className="font-semibold text-slate-950">{answered}/{total}</p>
          </div>
          <div className="rounded-lg bg-slate-50 p-3 ring-1 ring-slate-200">
            <p className="text-slate-500">Attempts</p>
            <p className="font-semibold text-slate-950">Unlimited retry</p>
          </div>
        </div>
      </div>
                ...
                {showResult ? (
                  <span
                    className={`rounded-full px-3 py-1 text-xs font-semibold ring-1 ${
                      correct ? "bg-green-50 text-green-800 ring-green-200" : "bg-amber-50 text-amber-900 ring-amber-200"
                    }`}
                  >
      ...
      {!certificate ? (
        <div className="sticky bottom-20 z-10 rounded-xl border border-slate-200 bg-white/95 p-4 shadow-lg backdrop-blur">
          {submitted ? (
            <div
              className={`mb-3 rounded-lg px-3 py-2 text-sm font-semibold ${
                passed ? "bg-green-50 text-green-800 ring-1 ring-green-200" : "bg-amber-50 text-amber-900 ring-1 ring-amber-200"
              }`}
              role="status"
            >
              {passed
          ...
          {!certificateReady ? <p className="mt-3 text-xs text-slate-500">Checking for an existing certificate...</p> : null}
        </div>
      ) : (
        <CommunityCertificatePanel certificate={certificate} courseTitle={courseTitle} />
      )}
    </section>
  );
}

function CommunityCertificatePanel({
  certificate,
  courseTitle,
}: {
  ...
  return (
    <section className="rounded-xl border border-teal-200 bg-gradient-to-b from-white to-teal-50 p-5 shadow-sm">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="text-xs font-bold uppercase tracking-wide text-teal-700">Certificate unlocked</p>
          <h2 className="mt-1 text-2xl font-semibold tracking-tight text-slate-950">Course passed</h2>
        </div>
      ...
      <div className="demo-certificate" id="demo-certificate">
        <div className="demo-certificate-inner">
          ...
          <p className="demo-certificate-copy">successfully completed and passed this e-learning course.</p>
          <dl>
            <div>
              <dt>Issued</dt>
            ...
            <div>
              <dt>Score</dt>
              <dd>{certificate.scoreLabel} ({certificate.scorePercent}%)</dd>
            </div>
            <div>
              <dt>CPD value</dt>
              <dd>{CERTIFICATE_CPD_HOURS} hours / {CERTIFICATE_CPD_POINTS} points</dd>
            </div>
            <div>
              <dt>Level</dt>
          ...
          <p className="demo-certificate-footer">Keep this certificate with your CPD record. Issued by {CERTIFICATE_PROVIDER}.</p>
        </div>
      </div>
    </section>
  );
